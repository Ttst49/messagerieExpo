...
export default function editGroup() {
  ...
  function getGroup(id: string) {
    axiosHttp
      .get(GlobalConstants.baseUrl + "group/conversation/show/" + id)
      .then((response) => {
    ...
    axiosHttp
      .get(
        GlobalConstants.baseUrl +
          "group/conversation/promote/admin/" +
          group?.id +
          "/" +
          id,
      )
  ...
  function demoteAdmin(id: number) {
    axiosHttp
      .get(
        GlobalConstants.baseUrl +
          "group/conversation/demote/admin/" +
          group?.id +
          "/" +
          id,
      )
        ...
        <FlatList
          ...
          renderItem={({ item }: { item: UserFull }) => (
            <DataTable.Row>
              ...
              {group?.adminMembers.some(
                (user) => user.username == item.relatedTo.username,
              ) ? (
                <Button
                  mode={"outlined"}
                  onPress={() => {
                    demoteAdmin(item.relatedTo.id);
                  }}
                >
                  Déstituer admin
                </Button>
                ...
                <Button
                  mode={"outlined"}
                  onPress={() => {
                    promoteAdmin(item.relatedTo.id);
                  }}
                >
                  Promouvoir admin
                </Button>
              ...
              {group?.owner.id == item.id ? (
                ...
                <Button
                  mode={"outlined"}
                  onPress={() => {
                    promoteOwner(item.relatedTo.id);
                  }}
                >
